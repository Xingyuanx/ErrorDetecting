...
| fault_level | VARCHAR | 20 | NOT NULL | 'medium' | 故障级别(low/medium/high/critical) |
| title | VARCHAR | 200 | NOT NULL | - | 故障标题 |
| description | TEXT | - | NULL | - | 故障详细描述 |
| affected_nodes | JSONB | - | NULL | - | 受影响的节点列表 |
| affected_clusters | JSONB | - | NULL | - | 受影响的集群列表 |
| root_cause | TEXT | - | NULL | - | 根本原因分析 |
| repair_suggestion | TEXT | - | NULL | - | 修复建议 |
| status | VARCHAR | 20 | NOT NULL | 'detected' | 状态(detected/analyzing/repairing/resolved/failed) |
...
- KEY idx_fault_records_type (fault_type)
- KEY idx_fault_records_status (status)
- KEY idx_fault_records_created_at (created_at)
...
### 3.1 外键关系
```sql
-- fault_records.cluster_id → clusters.id (SET NULL, CASCADE)
-- exec_logs.fault_id → fault_records.fault_id (CASCADE, CASCADE)
-- system_logs.fault_id → fault_records.fault_id (SET NULL, CASCADE)
...
- **clusters** ↔ **fault_records**: 一对多（可选），一个集群可能发生多个故障。
- **fault_records** ↔ **exec_logs**: 一对多，一个故障可能有多次修复执行记录。
- **fault_records** ↔ **system_logs**: 一对多（可选），一个故障可能关联多条系统日志。
